...
const AdminQuizzesPage = () => {
      ...
      if (willDelete) {
        deleteQuiz(dispatch, quiz.quizId, token).then((data) => {
          if (data.type === quizzesConstants.DELETE_QUIZ_SUCCESS) {
            swal(
              "Quiz Deleted!",
              `${quiz.title} successfully deleted`,
              "success"
            );
          } else {
            swal("Quiz Not Deleted!", `${quiz.title} not deleted`, "error");
  ...
  useEffect(() => {
    if (quizzes.length === 0) {
      fetchQuizzes(dispatch, token).then((data) => {
        // Ensure quizzes data is correctly fetched
        setQuizzes(data.payload.$values || []);
      });
    }
  }, [dispatch, token]);
    ...
    <div className="adminQuizzesPage__container">
      ...
      <div className="adminQuizzesPage__content">
        ...
        {quizzes.length === 0 ? (
          ...
          quizzes.map((quiz, index) => (
            quiz && quiz.category && ((catId && quiz.category.catId == catId) || (catId == null)) ? (
              <ListGroup
                className="adminQuizzesPage__content--quizzesList"
                key={index}
              >
                <ListGroup.Item className="align-items-start" action>
                  <div className="ms-2 me-auto">
                    <div className="fw-bold">{quiz.title || "Untitled Quiz"}</div>
                    <p style={{ color: "grey" }}>{quiz.category.title || "No Category"}</p>
                    <p className="my-3">{quiz.description || "No Description"}</p>
                    <div className="adminQuizzesPage__content--ButtonsList">
                      <div
                        onClick={() =>
                          questionsHandler(quiz.title, quiz.quizId)
                      ...
                      >{`Marks : ${(quiz.numOfQuestions || 0) * 5}`}</div>
                      <div
                        style={{
                          border: "1px solid grey",
                          width: "100px",
                      ...
                      >{`${quiz.numOfQuestions || 0} Questions`}</div>
                      <div
                        onClick={() =>
                          updateQuizHandler(quiz.title, quiz.quizId)
                        }
